...
public class NextGreaterElementI {
    ...
    private static int[] nextGreaterElement(int[] nums1, int[] nums2) {
        ...
        for (int i = 0; i < nums2.length; i++) {
            ...
            // 如果栈顶元素为null，或者当前元素小于等于栈顶元素
            if (top == null || nums2[i] <= nums2[top]) {
                stack.push(i);
                continue;
            }

            while (top != null && nums2[i] > nums2[top]) {
                Integer index = map.get(nums2[top]);
                if (index != null) {
